...
class StartTask(unittest.TestCase):
    ...
    def test_003_DownloadXML(self):
        ...
        try:
            ...
            with open(r"C:\test_data\\" + line + ".zip", "wb") as receive:
        ...
        name = ''
        print(unicode_name,'\n',name)
        xml_zip = zipfile.ZipFile(r"C:\test_data\\" + line + ".zip")
        for name in xml_zip.namelist():
            unicode_name = name.encode('cp437').decode('cp866')
            print(unicode_name)
        xml_zip.extract(name,r'C:\test_data\\')
        xml_zip.close()
        os.chdir(r'C:\test_data\\')
        try:
            os.path.exists(r'C:\test_data\53010.xml')
            os.remove(r'C:\test_data\53010.xml')
